...
% (1) Use a different score to select the merged seed region configuration.
% (2) Discard a seed region if the training accurracy of the corresponding class is low.
% (3) Exclude discarded seed regions in clustering.
...
% If C(i,i) is the highest, then include the pairs (i,j) where C(i,j)>=(C(i,j)*confusionratiothre).
...
% Two seed regions i and j are adjacent if the following conditions hold.
% (1) j is within ntoprankthre seed regions of i in terms of reducedrankscores and reciprocally.
% (2) reducedrankscores(i,j)<=reducedrankscorethre.
% (3) replacementratios(i,j)>=replacementratiothre and reciprocally.
% (4) localranksumspvals(i,j)<=localranksumpvalthre and reciprocally.
% (5) cocontributions(i,j)>=cocontributionthre and reciprocally.
% (6) neither i nor j are singletons.
% Find connected components of the graph.
% Add extra links according to the confusion matrix of result_for_original on training data.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% If importthre=0, then replacementratiothre, localranksumpvalthre, cocontributionthre are determined from the distributions of the scores.
% Otherwise the three threshold values are imported.

if (importthre==0)

 % Determine replacementratiothre, localranksumpvalthre, cocontributionthre.
 % Sort replacementratios and cocontributions in an ascending order and localranksumspvals in a descending order.
 % Select the top quantilethre values of the sorted values as threshold.

 vals1=replacementratios(:); vals1=sort(vals1);
 vals2=localranksumspvals(:); vals2=sort(vals2,'descend');
 vals3=cocontributions(:); vals3=sort(vals3);
 k=length(vals1); k=round(k*quantilethre);
 replacementratiothre=vals1(k);
 localranksumpvalthre=vals2(k);
 cocontributionthre=vals3(k);
...
for ind=1:ncs
 ...
 if (sum(sum(rG(subset,subset)==1))==0)
  ...
  %nn=max(bestpartition)+1;
  %bestpartition(subset)=nn; 
...
% (1) Use seedmarkergroupindicators to find the members in each component which are very distinct from other members within the same component.  Those members are erroneously placed in the corresponding components.  Discard those members.
% (2) If the discarded members pair with other members in subpartitions, then discarding those members will leave their partners to become singletons.  Assign those singleton partners to the best other subpartitions.
% (3) If a component incurs no subpartition, then try to find singletons from it.
% (4) If a component reports multiple subpartitions, then generate all combinations of the multiple subpartitions.
% (5) Report multiple merged seed region outcomes and apply CNN to cluster images accordingly.  Judge from the clustering outcomes, choose the best one in terms of intra and inter cluster similarities.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
