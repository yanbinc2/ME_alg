...
def ME_CNN(x_train, train_label, test_array, true_answer, Num_Classes):  
    ...
    model.add(Convolution2D(filters=6, kernel_size=(5, 5), strides=1, activation='relu', padding="same", data_format="channels_last", input_shape=x_train[0].shape))    # ((I+2P-F)/S)+1  =  ((28+2*0-5)/1)+1=24
    ...
    # 2
    model.add(AveragePooling2D(pool_size=(2, 2), strides=2, padding="valid"))   # ((24+2*0-2)/2)+1=12
