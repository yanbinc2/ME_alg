...
% datamode=1: Use data to generate seed regions.
% datamode=2: Use embedded projections of data to generate seed regions.
...
% In each iteration of picking the next seed region, do not try to replace the new candidate region for all of existing classes.  Instead only find the existing classes which are close to the new candidate region.  Since we use kNN, geometrical relations should suffice to determine the outcomes.
% Report a fixed number of seed regions.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Use the original data or embedded data to filter valid images.
...
if (filter==1)
 ...
 for n=1:nregions
  ...
  if (datamode==1)
   subdata=data(ss,:);
   inds=demarcate_center_zones(subdata,theta,theta2,sizethre); 

  else
   subdata=embeddata(ss,:);
   [nhighvals,inds]=demarcate_center_zones2(subdata);
   if (nhighvals>=50)
    inds=[];
   end
  end
...
if (filter==1)
 ...
 % For each region, calculate the silhouette values of its members.
 % Calculate the average distance of each point to all regions.
 avgD=zeros(nimages,nregions);

 for m=1:nregions
  s1=find(regionneighborlabels==m);
  tmpX=data(s1,:);
  tmpD=pdist(tmpX,'euclidean');
  tmpD=squareform(tmpD);
  vals=mean(tmpD);
  avgD(s1,m)=transpose(vals);
 end

 for m=1:nregions
  for n=(m+1):nregions
   s1=find(regionneighborlabels==m);
   s2=find(regionneighborlabels==n);
   tmpX=data([s1 s2],:);
   tmpD=pdist(tmpX,'euclidean');
   tmpD=squareform(tmpD);
   tmpD=tmpD(1:length(s1),(length(s1)+1):(length(s1)+length(s2)));
   vals1=mean(tmpD); vals2=mean(transpose(tmpD));
   avgD(s1,n)=transpose(vals2);
   avgD(s2,m)=transpose(vals1);
  end
 end

 % Calculate the silhouette values of all data points.
 sils=zeros(1,nimages);
 for i=1:nimages
  l=regionneighborlabels(i);
  val1=avgD(i,l);
  %val2=min(avgD(setdiff(1:nregions,l)));
  val2=min(avgD(i,setdiff(1:nregions,l)));
  if (val1<val2)
   val=1-val1/val2;
  elseif (val1>val2)
   val=val2/val1-1;
 ...
 % Calculate the fraction of negative silhouette values in each region.
 nsvs=zeros(1,nregions);
 for n=1:nregions
  ss=find(regionneighborlabels==n);
  nsvs(n)=sum(sils(ss)<0)/length(ss);
 end


 % Exclude the regions whose center zones comprise <= theta3 fraction of neighbors.
 validregions=zeros(1,nregions);
 fracs=zeros(1,nregions);
 for n=1:nregions
  n1=sum(bilabels(:,1)==n);
  n2=sum((bilabels(:,1)==n)&(bilabels(:,2)==1));
  val=n2/n1; fracs(n)=val;
  if (val>=theta3)
   validregions(n)=1;
  end
 end

 % Exclude the regions whose NSVs exceed a threshold.
 nsvthre=0.5;
 
 validregions(find(nsvs>nsvthre))=0;
...
elseif (filter==2)
 ...
 % For each combination of (image,embedded marker), report the rank of the embedded marker in the output of the image.
 markerranks=zeros(nimages,ndim);
 for i=1:nimages
  [Y,I]=sort(data(i,:),'descend');
  for j=1:ndim
 ...
 % For each embedded marker, count the number of images possessing <= topthre ranks.
 toprankcnts=zeros(1,ndim);
 for n=1:ndim
  toprankcnts(n)=sum(markerranks(:,n)<=topthre);
 end
...
%{
rblocksize=10; nrblocks=ceil(nregions/rblocksize);

for p=1:nrblocks
 r1=(p-1)*rblocksize+1; r2=min(r1+rblocksize-1,nregions);
 ginds=[];
 for r=r1:r2
  if (regionpairDmode==0)
   ss=find(regionneighborlabels==r);
  else
   ss=find(regiontraininglabels==r);
  end
  ginds=[ginds ss];
 end
 [a,linds]=ismember(ginds,neighborinds);
 nl=length(ginds);
 for n=1:nblocks
  i1=(n-1)*blocksize+1; i2=min(i1+blocksize-1,nimages);

  k=i2-i1+1; 
  X=data([ginds i1:i2],:);
  D=pdist(X); D=squareform(D);
  D=D(1:nl,(nl+1):(nl+k));
  for r=r1:r2
   if (regionpairDmode==0)
    ss=find(regionneighborlabels==r);
   ...
   [aa,rinds]=ismember(ss,ginds);
   rD=D(rinds,:);
   [Y,I]=sort(rD);
   subD=Y(1:nk,:);
   subI=I(1:nk,:);
   a=subI(:); b=repmat(1:k,nk,1); b=b(:); vals=subD(:);
   tmat=sparse(a,b,vals,length(rinds),k); tmat=transpose(tmat);
   imageneighborDs(i1:i2,linds(rinds))=imageneighborDs(i1:i2,linds(rinds))+tmat;
  end
 end
end

b=transpose(1:nneighbors); a=transpose(neighborinds);
tmpmat=sparse(a,b,(1e-3)*ones(1,nneighbors),nimages,nneighbors);
imageneighborDs=imageneighborDs+tmpmat;
%}
...
ncandsconsidered=5; nneighborclasses=3;

ncandsconsidered=1; nneighborclasses=3; 


ncandsconsidered=10; nneighborclasses=3; 
...
while (flag==1)
 ...
 for cind=1:ncandsconsidered
  ...
  % Debug
  fprintf('cind=%d, regionind=%d, distance=%f, minval=%f, minndiff=%d\n',cind,maxind,Y(cind),minval,minndiff);
 ...
 fprintf('region %d, mindiffratio=%f, minndiff=%d\n',maxind,minval,minndiff);
