...
def ME_CNN(x_train, train_label, test_array, true_answer, Num_Classes):  
    ...
    model.add(Convolution1D(filters=6, kernel_size=5, strides=1, activation='relu', padding="same", data_format="channels_last", input_shape=x_train[0].shape))    # ((I+2P-F)/S)+1  =  ((28+2*0-5)/1)+1=24
    ...
    # 3
    # Convolution layer 3
    model.add(Convolution1D(filters=16, kernel_size=5, strides=1, activation='relu', padding="valid", data_format="channels_last"))
    #model.add(Activation('relu'))
    #model.add(Dropout(0.5))   #0610
    ...
    # 5
    model.add(Convolution1D(filters=120, kernel_size=5, strides=1, activation='relu', padding="valid", data_format="channels_last"))
#    model.add(Activation('relu'))
